...
def prepare_db():
    ...
    with Session(engine) as sess:
        ...
        if not sess.exec(select(Account).where(Account.email == "avealov@umich.edu")).first():
            ...
            a2 = Account(email="smileyjo@umich.edu", password=password_context.hash("456"), followable=1)
            a3 = Account(email="joeysmiley33@gmail.com", password=password_context.hash("789"), followable=1)
            a4 = Account(email="joey@gmail.com", password=password_context.hash("321"), followable=1)
            sess.add(a1)
            sess.add(a2)
            sess.add(a3)
            sess.add(a4)
            ...
            sess.add(f1)
            sess.add(f2)
            m1 = Meal(email=a2.email, mealName="Chicken and Rice", category="chicken", dateMade=date(2023, 11, 1))
            m2 = Meal(email=a3.email, mealName="Chicken and Brocolli", category="chicken", dateMade=date(2023, 11, 2))
            m3 = Meal(email=a1.email, mealName="Macaroni and Cheese", category="pasta", dateMade=date(2023, 11, 4))
            m4 = Meal(email=a4.email, mealName="Ceasar Salad", category="salad", dateMade=date(2023, 11, 6))
            sess.add(m1)
            sess.add(m2)
            sess.add(m3)
            sess.add(m4)
            ...
            i3a = Ingredients(mealID = m3.mealID, ingredient="macaroni", quantity=4, unit="oz")
            i3b = Ingredients(mealID = m3.mealID, ingredient="shredded cheese", quantity=2, unit="oz")
            i4a = Ingredients(mealID = m4.mealID, ingredient="romaine lettuce", quantity=100, unit="grams")
            i4b = Ingredients(mealID = m4.mealID, ingredient="croutons", quantity=30, unit="grams")
            i4c = Ingredients(mealID = m4.mealID, ingredient="salad dressing", quantity=2, unit="tablespoons")
            sess.add(i1a)
            sess.add(i1b)
            sess.add(i2a)
            sess.add(i2b)
            ...
            sess.add(i4b)
            sess.add(i4c)
            mp1 = MealPlan(mealPlanDate=date(2023, 11, 12), email=a1.email)
            mp2 = MealPlan(mealPlanDate=date(2023, 11, 19), email=a3.email)
            sess.add(mp1)
            sess.add(mp2)
            sess.commit()
            ...
            d8 = MealPlanDay(mealID = m3.mealID, mealPlanID = mp1.mealPlanID)
            ...
            d13 = MealPlanDay(mealID = m3.mealID, mealPlanID = mp1.mealPlanID)
